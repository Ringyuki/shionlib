...
const calcPadding = (base: string, hasPrefix: boolean, hasSuffixOrClear: boolean): string => {
  return cn(base, hasPrefix && 'pl-7', hasSuffixOrClear && 'pr-12')
}
    ...
    return (
      <div data-slot="input-wrapper" className={cn('relative w-full min-w-0', className)}>
        ...
        <input
          ...
          data-slot="input"
          className={cn(
            'bg-input/30 file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground border-input flex w-full min-w-0 rounded-md border md:text-sm text-base shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
            baseClass,
