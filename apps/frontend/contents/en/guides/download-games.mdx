...
Click the **paper plane icon** button to open the push menu, which lists every push target you currently have enabled. Pick one and the download is handed over to that client.

| Push target      | What it does                                                    |
| ---------------- | --------------------------------------------------------------- |
| **Aria2**        | Pushes to a locally or remotely running Aria2 download manager  |
| **LunaBox**      | Hands the download to LunaBox through the `lunabox://` protocol |
| **ReinaManager** | Hands the download to ReinaManager through `reinamanager://`    |

Before using Aria2, configure the connection details (protocol, host, port, secret, download path) in **Account Settings → Download Settings**. If the connection fails, an error appears and you are directed to the settings page — the gear icon next to the Aria2 entry in the push menu takes you there as well.

LunaBox and ReinaManager can each be toggled in **Account Settings → Download Settings**; a disabled one no longer appears in the push menu. ReinaManager relies on a Bangumi ID to aggregate metadata, so the entry is unavailable for game entries that have no Bangumi ID.
...
Click the **<Ellipsis />** menu in the bottom-right of a resource card and select **Update History** to view all historical versions of that resource, including file size, hash, update time, and the uploader's update reason.

## Reporting a Resource

If you find a resource that violates rules (e.g., malware, copyright issues), click the **<Ellipsis />** menu and select **Report Violation**, then fill in the reason. Malicious reports will be handled accordingly.
