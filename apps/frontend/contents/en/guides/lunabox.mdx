...
description: 'Learn how to enable LunaBox in the Shionlib push menu and send game downloads directly to the LunaBox desktop app.'
date: 2026-03-11
author_uid: 1
...
- **Installer (setup)**: Recommended for most users. During installation, LunaBox will usually try to register the `lunabox://` protocol automatically, so pushing should work out of the box. Data is stored in `%APPDATA%\LunaBox` by default.
...
3. Turn off **Show LunaBox in the push menu**
...
3. Click the **paper plane icon** button on the right of the file row to open the push menu
4. Choose **LunaBox** to push the download to it
5. If a Turnstile verification is required, complete it in the popup — LunaBox will receive the task automatically once verified
6. Switch to LunaBox — the download will have started
